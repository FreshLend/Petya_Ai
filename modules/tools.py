    ...
    try:
        ...
        for i in range(0, len(emojis), 10):
            ...
            for emoji in page_emojis:
                ...
                emoji_str = str(emoji)
                emoji_list.append(f"{status} {emoji_str} - `:{emoji.name}:` (ID: {emoji.id})")
...
async def ping(interaction: discord.Interaction):
    ...
    await interaction.response.send_message(f'🏓Pong! {int(latency)} мс')
...
async def reboot_command(interaction: discord.Interaction):
    ...
    await interaction.response.send_message(f"Бот перезагружается... Ожидайте завершения операций (максимум {config.SHUTDOWN_TIME} секунд).", ephemeral=True)
...
async def shutdown_command(interaction: discord.Interaction):
    ...
    await interaction.response.send_message(f"Бот выключается... Ожидайте завершения операций (максимум {config.SHUTDOWN_TIME} секунд).", ephemeral=True)
...
async def cipher(
                ...
                if char.isalpha():
                    ...
                    if self._is_russian(char):
                        ...
                        if self._is_russian(key_char):
                            if key_char.isupper():
                                key_alphabet = self.russian_upper
                            else:
                                key_alphabet = self.russian_lower
                        else:
                            if key_char.isupper():
                                key_alphabet = self.english_upper
                            else:
                                key_alphabet = self.english_lower
    ...
    try:
        ...
        if cipher_type == "caesar":
            ...
            cipher_name = "Виженер"
            if not key:
                await interaction.response.send_message("❌ Для шифра Виженера требуется ключ!", ephemeral=True)
                return
            if action == "encrypt":
            ...
            cipher_name = "XOR"
            if not key:
                await interaction.response.send_message("❌ Для XOR шифра требуется ключ!", ephemeral=True)
                return
            if action == "encrypt":
        ...
        embed = discord.Embed(
            title=f"{action_emoji} {cipher_name} - {action_text}",
            color=discord.Color.blue()
        )
        ...
        if cipher_type in ["md5", "sha1", "sha256", "sha512"]:
            embed.add_field(
                name="⚠️ Внимание", 
                value="Это хэш-функция - результат не может быть расшифрован!", 
                inline=False
            )
...
@bot.tree.command(name="check_imports", description="Проверить все .py файлы на наличие недостающих библиотек")
@app_commands.allowed_contexts(guilds=True, dms=True, private_channels=True)
@app_commands.user_install()
@app_commands.default_permissions(administrator=True)
async def check_imports_command(interaction: discord.Interaction):
    await interaction.response.defer(ephemeral=True, thinking=True)
    ...
    def extract_imports(content: str) -> List[str]:
        """Извлекает все импорты из Python кода"""
        imports = set()
        lines = content.split('\n')
    ...
    def check_module(module_name: str) -> Tuple[bool, str]:
        """Проверяет, установлен ли модуль. Любая ошибка импорта = не хватает библиотеки"""
        try:
    ...
    def scan_directory(directory: str, relative_path: str = "") -> List[str]:
        """Сканирует директорию на наличие .py файлов"""
        py_files = []
        ...
        for item in os.listdir(full_path):
            ...
            if os.path.isdir(item_path):
                if item not in ['__pycache__', 'venv', 'env', '.git', 'data', 'avatars']:
                    py_files.extend(scan_directory(directory, rel_item_path))
            elif item.endswith('.py'):
                py_files.append(rel_item_path)
        
        return py_files
    
    embed = discord.Embed(
        title="🔍 Проверка импортов Python",
        description="Анализ всех .py файлов проекта...",
        color=discord.Color.blue()
    )
    ...
    for item in ['main.py', 'config.py']:
        if os.path.exists(os.path.join(root_dir, item)):
            all_py_files.append(item)
    
    for module_dir in ['modules', 'plugins']:
        module_path = os.path.join(root_dir, module_dir)
        if os.path.exists(module_path):
            all_py_files.extend(scan_directory(module_path))
    
    total_files = len(all_py_files)
    
    for idx, py_file in enumerate(all_py_files):
        file_path = os.path.join(root_dir, py_file)
        ...
        for module in imports:
            success, error_msg = check_module(module)
            if success:
                file_success.append(module)
            else:
                file_errors.append({
                    "module": module,
                    "error": error_msg
        ...
        if file_errors:
            results["failed"].append({
                "file": py_file,
                "success_imports": file_success,
                "failed_imports": file_errors
            })
        else:
            results["success"].append({
                "file": py_file,
                "imports": file_success,
                "message": "OK"
            })
        ...
        if (idx + 1) % 10 == 0 or idx + 1 == total_files:
            progress_embed = discord.Embed(
                title="🔍 Проверка импортов Python",
                description=f"Прогресс: {idx + 1}/{total_files} файлов\n"
                           f"✅ Успешно: {len(results['success'])}\n"
                           f"❌ Ошибок: {len(results['failed'])}\n"
                           f"⚠️ Всего проблем: {results['total_errors']}",
                color=discord.Color.blue()
            )
            await interaction.edit_original_response(embed=progress_embed)
    
    result_embed = discord.Embed(
        title="📊 Результаты проверки импортов",
        description=f"**Проверено файлов:** {results['files_checked']}\n"
                   f"**✅ Успешно:** {len(results['success'])}\n"
                   f"**❌ С ошибками:** {len(results['failed'])}\n"
                   f"**⚠️ Всего проблем:** {results['total_errors']}",
        color=discord.Color.green() if results['total_errors'] == 0 else discord.Color.red()
    )
    
    if results["failed"]:
        failed_text = ""
        for fail in results["failed"][:15]:
            file_name = fail["file"]
            failed_text += f"\n**📄 {file_name}**\n"
            
            if "failed_imports" in fail:
                for imp in fail["failed_imports"][:5]:
                    error_msg = imp["error"][:80]
                    failed_text += f"  ❌ `{imp['module']}` - {error_msg}\n"
            
            if len(failed_text) > 900:
                failed_text += "\n... и ещё файлы\n"
                break
        
        if failed_text:
            result_embed.add_field(
                name=f"❌ Файлы с ошибками ({len(results['failed'])})",
    ...
    for fail in results["failed"]:
        if "failed_imports" in fail:
            for imp in fail["failed_imports"]:
                missing_modules.add(imp["module"])
    
    if missing_modules:
        install_command = "pip install " + " ".join(sorted(missing_modules))
        result_embed.add_field(
            name="📦 Установка недостающих библиотек",
            value=f"```bash\n{install_command}\n```",
            inline=False
        )
    
    result_embed.set_footer(text=f"Команда выполнена: {datetime.now().strftime('%d.%m.%Y %H:%M:%S')}")
